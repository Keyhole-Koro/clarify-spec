...
* 精度安定化の正本: `organize/specs/pipeline/a1-atom-stability.md`
...
* **Gemini Flash** — claim 正規化と `kind/confidence` 付与に限定

## 4. 2段階処理

1. Stage 1: claim boundary を決定論的に切る
2. Stage 2: Gemini が candidate ごとに正規化する

Gemini は atom 数を増減させない。
...
## 6. 出力スキーマ: `workspaces/{workspaceId}/topics/{topicId}/atoms/{atomId}`
...
erDiagram
  ATOM {
    ...
    int candidateIndex "Stage 1 の出現順"
...
* `atomId` は `sha256(topicId + sourceInputId + candidateIndex + normalizedClaim)` で決定論的に生成する
* 同一 `sourceInputId` からの再抽出時は `candidateIndex + normalizedClaim` を使って重複判定する
...
| 確信度 | 条件 |
| --- | --- |
| 0.95 | 明確な事実記述。出典が明確で検証可能 |
| 0.8 | 一般に受け入れられている知識 |
| 0.6 | 文脈依存の判断や限定的な主張 |
| 0.4 | 推測・意見・条件付きの主張 |
| 0.2 | 曖昧・情報不足・矛盾の可能性あり |

## 9. LLM プロンプト（Structured Output）

> あなたは情報分析の専門家です。以下の claim candidate を、Atom 用に正規化してください。
>
> **入力 candidate:** {claimCandidate}
>
> **各Atomの出力フィールド:**
> - `title`: 20文字以内の要約見出し
...
> - `confidence`: `0.95` / `0.8` / `0.6` / `0.4` / `0.2`
> - `reject`: atom 化すべきでない場合は true
>
> **ルール:**
> - candidate 数を増減させない
> - 原文にない推測を追加しない
