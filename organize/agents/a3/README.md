...
* Output: `workspaces/{wid}/topics/{tid}/outlines/{outlineVersion}`, `workspaces/{wid}/topics/{tid}/nodes/*`, `workspaces/{wid}/topics/{tid}/edges/*`, `workspaces/{wid}/topics/{tid}/nodes/{nid}/evidence/*`
...
3. **Evidence Upsert** — 各 claim の `sourceAtomIds` を evidence として `nodes/{nodeId}/evidence/{evidenceId}` に書き込む
4. **仮 `context_summary` 生成（CREATE 時のみ）** — A7 NodeRollup が非同期で完了するまでの間、Assembly が空のノードをスキップしないよう仮の要約を書く:
   * claim の `normalizedClaim` の先頭 100 トークンを切り出す
   * 先頭に `{title} ({kind}): ` を付加する
   * GCS に保存して `context_summary_ref` を設定する
   * A7 が rollup を生成した時点で上書きされる
5. **Edge Upsert** — 確定した関係を `edges/{edgeId}` に書き込む（フィールド: `source_id`, `target_id`, `relation`, `order_key`）
6. **Outline 再構成** — 下記アルゴリズムで再構成する
7. **Events Emit** — `outline.updated`、変更 node ごとに `topic.node_changed`、DEFER 分は `atom.reissued`
...
**`atom.reissued` の payload:**

| フィールド | 説明 |
| --- | --- |
| `topicId` | 対象 topic |
| `atomId` | 対象 atom |
| `reason` | DEFER / consistency_error / schema_mismatch 等 |
| `reissueCount` | 何回目の再発行か（0始まり、インクリメントする） |
| `previousDecision` | 前回の判定結果 |
| `previousReason` | 前回の判定理由 |

**無限ループ防止:** `reissueCount >= 3` の場合は再評価せず強制 REJECT とし、`reject_reason: max_reissue_exceeded` をログに記録する。
