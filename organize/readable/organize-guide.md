...
`topic` は、1 回の質問ではなく、継続的に育てる 1 テーマの知識単位である。

判断基準:

* 同じ中心対象や root question を共有する
* 同じ node / edge 群を再利用したい
* 同じ knowledge schema で整理できる
* 数日から数か月かけて更新されても自然

逆に、明らかに別テーマ・別寿命・別 access boundary なら別 topic に切る。
...
以下で `topic` 関連の Firestore 物理パスを書くときは、`firestore/schema.md` に従って
`workspaces/{workspaceId}/topics/{topicId}/...` を正本とする。
...
flowchart TD
  ...
  A3 --> T[Topic Graph<br>workspaces/{workspaceId}/topics/{topicId}/nodes, edges]
  A4 --> I[Index Items]
  A7 --> R[Node Rollup]
```
...
atom 群をどの topic に所属させるかを自動判定する前段。

* やること:
  * 既存 topic 候補を retrieval する
  * 候補 topic との関連度を scoring する
  * 上位候補だけ Gemini に渡して既存 attach / 新規作成を最終判定する
* 方針:
  * 誤って既存 topic を汚染するより、新規 topic を切る方を安全側とする
* 次へ渡すもの:
  * `topic.resolved`

### 4. A2 DraftAppender

解決済み topic に対して draft を進める。
...
* やること:
  ...
  * 今の topic で必要な node type / relation type / attribute を見て schema 変更要否を判定する
  * 必要なら topic schema を 1 version 進める
  * Firestore `workspaces/{workspaceId}/topics/{topicId}/pipelineBundles/{bundleId}` を作る
* 次へ渡すもの:
  * `bundle.created`
  * 必要なら `topic.schema_updated`

### 6. A6 BundleDescription

bundle を人間が読みやすい説明にする補助 Agent。

* やること:
  * bundle の説明 HTML を GCS に出す
  * Firestore `workspaces/{workspaceId}/topics/{topicId}/pipelineBundles/{bundleId}.descRef` を更新する
* 主目的:
  * デバッグしやすくする
...
* やること:
  * outline を更新する
  * `workspaces/{workspaceId}/topics/{topicId}/nodes/*` を upsert する
  * `workspaces/{workspaceId}/topics/{topicId}/edges/*` を upsert する
...
* やること:
  ...
  * `workspaces/{workspaceId}/topics/{topicId}/nodes/{nodeId}.rollupRef` を更新する
* 使い道:
...
`Organize` では topic ごとに knowledge schema が進化してよい。  
ここでいう schema は DB の物理スキーマではなく、その topic をどういう node / relation / attribute で整理するかという知識構造である。
...
* `topicId` 境界
* Firestore/GCS/Act memory の責務分割
* 認可境界
* 物理パス
* version / CAS / idempotency

```mermaid
flowchart LR
  D[Draft Delta] --> DIST[A3b Distill]
  DIST --> DECIDE{Schema change needed?}
  DECIDE -- No --> APPLY[A3 Cleaner]
  DECIDE -- Yes --> SCHEMA[topic schema v+1]
  SCHEMA --> APPLY
  APPLY --> GRAPH[Nodes / Edges / Outline]
```

イメージとしては、

* topic A は `concept / method / benchmark` を持つ
* topic B は `person / org / event / claim` を持つ

のように、整理軸が topic ごとに違ってよい。

### Topic Schema の保存先

Firestore:

* `workspaces/{workspaceId}/topics/{topicId}.schemaVersion`
* `workspaces/{workspaceId}/topics/{topicId}/schemas/{version}`

`schemas/{version}` には次のようなものを置く。

```json
{
  "version": 3,
  "nodeKinds": ["concept", "component", "metric"],
  "relationTypes": ["has_component", "measured_by", "depends_on"],
  "attributeDefs": {
    "metric.unit": "string",
...
### TopicResolver / A2 DraftAppender の成果物例

Firestore:

```json
{
  "path": "workspaces/ws_demo/topics/tp_ai_agents",
...
```json
{
  "path": "workspaces/ws_demo/topics/tp_ai_agents/pipelineBundles/pb_004",
  "topicId": "tp_ai_agents",
...
```json
{
  "path": "workspaces/ws_demo/topics/tp_ai_agents/nodes/nd_tool_use",
  "kind": "concept",
  "title": "Tool Use",
...
```json
{
  "path": "workspaces/ws_demo/topics/tp_ai_agents/edges/ed_001",
...
* 中核規約:
  ...
  * `organize/specs/pipeline/a1-atom-stability.md`
  * `organize/specs/pipeline/phase-runner-and-inspector.md`
  * `organize/specs/pipeline/run-phase-contract.md`
  * `organize/specs/pipeline/minimal-fixture-set.md`
